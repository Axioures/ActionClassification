function [] = create_cmd_codebook(configFile, code_path, matlab_exec)
...
for s = 1:num_splits
    inner_cmd = sprintf('minimal_codebook(params, db_params, %d);', s);
    cmd{s} = [matlab_exec, prefix, '"', add_path, main_config, db_config, inner_cmd, 'exit', '"'];
end

fid = fopen(sprintf('./cg_%s_%s.txt', params.dbName, params.experiment_string), 'w');
fprintf(fid, '%s\n', cmd{:});
fclose(fid);
end
