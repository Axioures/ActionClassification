function encoded_features = feature_encoding_wrapper_SemSeg(features, features_clean, params, clip_name)
    if (params.reuse.encodings) && exist('clip_name', 'var')
        ...
        switch params.encoding.type
            case 'BoVW'
                encoded_features = encode_multichannel_bof(features, features_clean, params);
            case 'fisher'
                encoded_features = encode_multichannel_fisher(features, features_clean, params);
            case 'vlad'
...
function encoded_features = encode_multichannel_bof(features, features_clean, params)
	c = params.channels;
    ...
    for i=1:length(c)
        ...
        elseif params.features.segm
            encoded_features.(c(i).name) = BagOfFeaturesSemSeg(features.(c(i).descriptor), features_clean.(c(i).descriptor), codebook);
        else
...
function encoded_features = encode_multichannel_fisher(features, features_clean, params)
    c = params.channels;
    encoded_features = cell2struct(cell(length(c),1), {c.name}, 1);
    if params.features.segm
        for i=1:length(c)
            gmm = parload(fullfile(params.paths.training, sprintf('codebook_%s.mat', c(i).name)));
            encoded_features.(c(i).name) = ...
                [encode_fisher(features.(c(i).name), gmm) encode_fisher(features_clean.(c(i).name), gmm)] ;
        end        
    else
        for i=1:length(c)
            gmm = parload(fullfile(params.paths.training, sprintf('codebook_%s.mat', c(i).name)));
...
function bof = BagOfFeaturesSemSeg(features, features_clean, codebook)
	bof_all = BagOfFeatures(codebook, features);
    bof_clean = BagOfFeatures(codebook, features_clean);
    bof = [bof_all, bof_clean];
% 	bof = bof/(sqrt(bof*bof')+eps);
end
