function [] = exp_bow_arms_kernel(configFile)
...
%     keyboard
	...
	for s=1:length(testing_subjects)
        ...
        params.paths.encodings_arm1 = fullfile(params.paths.encodings_root_arm_1, sprintf('test_p%d', testing_subjects(s)));
        params.paths.encodings_arm2 = fullfile(params.paths.encodings_root_arm_2, sprintf('test_p%d', testing_subjects(s)));
		...
		if params.state<2			
            for i=1:length(training_clips)
				...
				[f,f_human, f_arm1, f_arm2] = feature_extraction_wrapper_SemSeg(training_clips(i), params);
				erase_msg(msg_size);
